...
void rpcserver_client_thread(void* arg){
    ...
    if(get_rpcmsg(&gotmsg,thrd->client_fd) == 0 && gotmsg.msg_type == AUTH && gotmsg.payload != NULL){
        ...
        if(is_authed){
            ...
            repl.payload = malloc((repl.payload_len = type_buflen(&uniq)));
            assert(repl.payload);
            type_to_arr(repl.payload,&uniq);
            free(uniq.data);
            int iserror = 0;
            if(send_rpcmsg(&repl,thrd->client_fd) != 0) iserror = 1;
            repl.msg_type = 0;
            free(repl.payload);
            repl.payload = NULL;
            repl.payload_len = 0;
            ...
            while(thrd->serv->stop == 0){
                ...
                switch(gotmsg.msg_type){
                    case LSFN:
                                    ...
                                    repl.msg_type = LSFN;
                                    if(send_rpcmsg(&repl,thrd->client_fd) != 0) {free(repl.payload);goto exit;}
                                    free(repl.payload);
                                    break;
                    case PING:
                                    free(gotmsg.payload);
                    ...
                    case CALL:
                                    ...
                                    if(cfn == NULL){
                                        ...
                                        if(send_rpcmsg(&repl,thrd->client_fd) != 0) goto exit;
                                        break;
...
exit:
    ...
    send_rpcmsg(&lreply,thrd->client_fd);
